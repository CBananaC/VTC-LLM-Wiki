...
- `02 Text and Tables/stroke_rehab_visual_locations_and_tables_full_generated.json` — complete visual-location map plus selectively reconstructed table cells, drawn cell geometry, captions, extraction methods, fallback text lines, and page provenance.
...
- `03 Analysis/stroke_rehab_chapter_01_full_text_structure_generated.json` — complete Chapter 1 JSON containing page text, logical paragraphs/list items, outline mapping, all visual locations, and 20 reconstructed table records.

All outputs are `generated_not_verified`. The raw embedded-text layer can contain source text objects from visual regions; use the clean reading-order layer or the Chapter 1 export for text-only study retrieval. Cross-page merges retain all contributing page and source-line IDs, but the current page mapper does not yet split multiple headings that begin on the same PDF page. Table reconstruction uses embedded PDF text and geometry, not visual OCR, and must be checked against rendered source pages. Charts, graphs, illustrations, and other non-table visuals have not been content-reconstructed. Summary, keyword extraction, and retrieval indexing have not yet been created.
